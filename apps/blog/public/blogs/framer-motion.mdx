...
<C>
  You'd create CSS styles for fading, handle state changes, and use the
  <L href="#IntersectionObserver">IntersectionObserver</L> API. It's effective
  but requires several steps and can get a bit verbose.
</C>

<C>First, Create the fade in style</C>
...
<C>
  But you don't need the state to change when the element mounts, instead you
  want it to fade in when it comes in view, to make it happen I'll be using the
