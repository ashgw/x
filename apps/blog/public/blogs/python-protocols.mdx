...
<C>
  ...
  <L href="https://mypy.readthedocs.io/en/stable/">``MyPy``</L> shows complaints{" "}
</C>
<Code
  code={`(.venv) ➜ mypy main.py 
...
<C>But when you fix it</C>
<Code
  code={`H: OrganicOrganism = Human()
# O: Human = OrganicOrganism() 
...
@runtime_checkable
class Bar(Protocol):
async def **call**(self, foo: Foo) -> None:
...
`}
language="python"
showLineNumbers={false}
