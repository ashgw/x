...
<C>
  The biggest drawback is that it can obscure the readability of a component's
  signature. When a function has multiple parameters, it is harder to discern
  the purpose of each parameter. This can become even more confusing when
  ``ReactElement`` is not explicitly defined in an interface or type.
</C>
